    ...
    public class TetrisStyler : Styler
    {
        ...
        public TetrisStyler(int canvas_length, float margin, MarginMode margin_mode, string json_path) : base()
        {
            ...
            if (margin_mode == MarginMode.PIXEL)
            {
                ...
                canvas_height = (int) ((TopGridRowNumber + BottomGridRowNumber + JsonInstance.Size) * step_length +
                                       margin * 2);
            }
            else
            {
                ...
                canvas_height = (int) ((TopGridRowNumber + BottomGridRowNumber + JsonInstance.Size + margin * 2) *
                                       step_length);
            }
            ...
            //Initialation
            __InitStyleWithMarginMode(canvas_width, canvas_height, margin_length, margin_top, margin_length,
                margin_bottom, MarginMode.PIXEL, JsonInstance.Matrix, JsonInstance.Size);
        ...
        private void UpdateTopCellColors()
        {
            ...
            sample_paint.DrawImage(sample, new Rectangle(0, 0, sample_back.Width, sample_back.Height),
                new Rectangle(0, 0, sample.Width, sample.Height), GraphicsUnit.Pixel);
            Point center_point;
        ...
        //public methods
        public void InitStyle(string i_folder, string i_point_path, string i_top_sample_path,
            string i_bottom_sample_path, Color i_canvas_color)
        {
        ...
        public override void Draw()
        {
            ...
            for (int i = 0; i < TopMatrix.GetLength(0); i++)
            {
                for (int j = 0; j < TopMatrix.GetLength(1); j++)
                {
                    if (TopMatrix[i, j] == CellColor.BLACK)
                    {
                        ...
                        paint.DrawImage(point_img, new Rectangle(position.X, position.Y, TetrisStep, TetrisStep),
                            new Rectangle(0, 0, point_img.Width, point_img.Height), GraphicsUnit.Pixel);
                    }
                }
            }
            paint = Graphics.FromImage(top_layer);
            paint.DrawImage(TopCanvas, new RectangleF(TopPosintion.X, TopPosintion.Y, TopSize.Width, TopSize.Height),
                new Rectangle(0, 0, TopCanvas.Width, TopCanvas.Height), GraphicsUnit.Pixel);
            ...
            paint.DrawImage(BottomCanvas,
                new RectangleF(BottomPosition.X, BottomPosition.Y, BottomSize.Width, BottomSize.Height),
                new Rectangle(0, 0, BottomCanvas.Width, BottomCanvas.Height), GraphicsUnit.Pixel);
            ...
            foreach (var b in bc)
            {
                paint.DrawImage(point_img, GetCellRectangle(b.Position.Row, b.Position.Column),
                    new Rectangle(0, 0, point_img.Width, point_img.Height), GraphicsUnit.Pixel);
            }
            paint = Graphics.FromImage(black_layer);
            paint.DrawImage(black, new RectangleF(CodePosition.X, CodePosition.Y, CodeSize.Width, CodeSize.Height),
                new Rectangle(0, 0, black.Width, black.Height), GraphicsUnit.Pixel);
